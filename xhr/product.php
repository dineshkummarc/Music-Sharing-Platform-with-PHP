    ...
    if (!empty($_POST['word'])) {
        ...
        if (!empty($results)) {
            ...
            foreach ($results as $key => $value) {
                $html .= '<div class="serach_trend" onclick="AddRelated(this,'.$value->id.')"><a href="javascript:void(0)">'.$value->title.'</a></div>';
            }
            $data['status'] = 200;
            $data['html'] = $html;
        }
        ...
        $results = $db->where('user_id',$music->user->id)
                      ->orderBy('id', 'DESC')
                      ->get(T_SONGS);
        if (!empty($results)) {
            $html = "";
    ...
    if (!empty($_POST['title']) && !empty($_POST['desc']) && !empty($_POST['tags']) && !empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0 && !empty($_POST['price']) && is_numeric($_POST['price']) && $_POST['price'] > 0 && !empty($_POST['units']) && is_numeric($_POST['units']) && $_POST['units'] > 0 && !empty($_POST['related']) && !empty($_POST['category']) && in_array($_POST['category'], array_keys($music->products_categories))) {
        ...
        if (empty($data['message'])) {
            ...
            if (!empty($product->id)) {
                ...
                $data['status'] = 200;
                $data['message'] = lang('Your product has been edited successfully');
...
if ($option == 'product_search') {
    $category = (isset($_GET['category']) && in_array($_GET['category'], array_keys($music->products_categories))) ? $_GET['category'] : '';
    $price_from = (isset($_GET['price_from']) && is_numeric($_GET['price_from'])) ? $_GET['price_from'] : 1;
    $price_to = (isset($_GET['price_to']) && is_numeric($_GET['price_from'])) ? $_GET['price_to'] : 10000;
    $text = "";
            ...
            if (!empty($music->items)) {
                foreach ($music->items as $key => $music->item) {
                    ...
                    if ($music->item->units <= $product->units) {
                        ...
                        $data['message'] = lang("Some products don't have enough of units");
                ...
                if (!empty($insert)) {
                    foreach ($insert as $key => $value) {
                        ...
                        $hash_found = $db->where('hash_id',$hash_id)->getValue(T_ORDERS,'COUNT(*)');
                        ...
                        foreach ($value as $key2 => $value2) {
                            ...
                            $db->where('id', $music->user->id)->update(T_USERS, array('wallet' => $db->dec($total)));
...
if ($option == 'get_cart') {
    $html = '';
    $carts = $db->where('user_id',$music->user->id)->orderBy('id', 'DESC')->get(T_CARD,5);
    if (!empty($carts)) {
...
if ($option == 'tracking') {
    if (!empty($_POST['tracking_url']) && !empty($_POST['tracking_id']) && !empty($_POST['hash']) && pt_is_url($_POST['tracking_url'])) {
        ...
        if (!empty($order)) {
            ...
            $data['status'] = 200;
            $data['message'] = lang('Tracking info has been saved successfully');
        }
        else{
            $data['message'] = lang('Order not found');
        }
        ...
        if (!empty($music->purchase)) {
            ...
            if (!empty($music->orders)) {
                ...
                foreach ($music->orders as $key => $music->order) {
                    ...
                    $music->html .= '<tr><td><h6 class="mb-0">'.$music->order->product->title.'</h6></td><td>'.number_format(($music->order->price/$music->order->units),2,'.').'</td><td>'.$music->order->units.'</td><td><span class="font-weight-semibold">'.$music->config->currency_symbol.number_format($music->order->price,2,'.').'</span></td></tr>';
                }
                $music->product_owner = userData($music->user_id);
                $music->address = $db->where('id',$music->address_id)->getOne(T_ADDRESS);
                $music->total = number_format($music->total,2,'.');
                $music->total_commission = number_format($music->total_commission,2,'.');
                $music->total_final_price = number_format($music->total_final_price,2,'.');
