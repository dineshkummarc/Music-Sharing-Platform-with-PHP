...
if (in_array(true, $request)) {
    ...
    $shell = shell_exec(
        "$ffmpeg_b -y -i $audio_output_mp3 -filter_complex \"aformat=channel_layouts=mono,showwavespic=s=1100x150:colors=#474747\" -frames:v 1 $audio_output_black_wave 2>&1"
    );
    ...
    $shell = shell_exec(
        "$ffmpeg_b -y -i $audio_output_mp3 -filter_complex \"aformat=channel_layouts=mono,showwavespic=s=1100x150:colors=#bfbfbf\" -frames:v 1 $audio_output_day_wave 2>&1"
    );

    $shell = shell_exec(
        "$ffmpeg_b -y -i $audio_demo_output_mp3 -filter_complex \"aformat=channel_layouts=mono,showwavespic=s=1100x150:colors=#474747\" -frames:v 1 $audio_output_demo_black_wave 2>&1"
    );
    ...
    $shell = shell_exec(
        "$ffmpeg_b -y -i $audio_demo_output_mp3 -filter_complex \"aformat=channel_layouts=mono,showwavespic=s=1100x150:colors=#bfbfbf\" -frames:v 1 $audio_output_demo_day_wave 2>&1"
    );
