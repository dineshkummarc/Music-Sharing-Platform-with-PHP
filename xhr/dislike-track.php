...
if (!isdisLiked($getSong->id)) {

    $delete_like = $db->where('user_id', $user->id)->where('track_id', $getSong->id)->where('comment_id',0)->delete(T_LIKES);
    if ($delete_like) {
	...
	$create_like = $db->insert(T_DISLIKES, ['user_id' => $user->id, 'track_id' => $getSong->id, 'time' => time()]);
	if ($create_like) {
        ...
        RecordUserActivities('dislike_track',array('track_user_id' => $getSong->user_id, 'audio_id' => $getSong->audio_id));
		$data['status'] = 200;
	}
} else {
	$delete_like = $db->where('user_id', $user->id)->where('track_id', $getSong->id)->where('comment_id',0)->delete(T_DISLIKES);
	if ($delete_like) {
