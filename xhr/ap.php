...
if ($option == 'remove_multi_product_category') {
    $data['status'] = 400;
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                $cat = $db->where('id',Secure($value))->getOne(T_PRODUCTS_CATEGORY);
                if (!empty($cat)) {
                    $data['status'] = 200;
                    $db->where('lang_key',$cat->lang_key)->delete(T_LANGS);
                    $db->where('id',$cat->id)->delete(T_PRODUCTS_CATEGORY);
                }
            }
        }
    }
    $types = array('product' => T_PRODUCTS_CATEGORY, 'song' => T_CATEGORIES);
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        $cat = $db->where('id',Secure($_POST['id']))->getOne($types[$_GET['type']]);
        if (!empty($cat)) {
            $data['status'] = 200;
            $db->where('lang_key',$cat->lang_key)->delete(T_LANGS);
            $db->where('id',$cat->id)->delete($types[$_GET['type']]);
        }
    }
...
if ($option == 'debug-ffmpeg') {

    $ffmpeg_b                   = $music->config->ffmpeg_binary_file;
    $audio_output_mp3 = dirname(__DIR__) . "/admin-panel/audio/test_converted.mp3";
    @unlink($audio_output_mp3);
    $audio_file_full_path = dirname(__DIR__) . "/admin-panel/audio/test.mp3";
    ...
    $shell = shell_exec(
        "$ffmpeg_b -y -i $audio_file_full_path -map 0:a:0 -b:a " .
            $convert_speed .
            "k $audio_output_mp3 2>&1"
    );
    if (file_exists($audio_output_mp3)) {
        $data['video_url'] = $site_url . '/admin-panel/audio/test_converted.mp3';
    }
    if (!isFuncEnabled('shell_exec')) {
       $data['video_url'] = '';
       $shell = 'Warning: shell_exec() has been disabled for security reasons, please contact your hosting provider to enable it.';
    }
    $data['status'] = 200;
    $data['data']   = $shell;
    header("Content-type: application/json");
...
if ($option == 'setup_google_drive') {
    $data['status'] = 400;
    require './assets/import/google/vendor/autoload.php';
    try {
...
if ($option == 'reset_google_drive') {
    $data['status'] = 200;
    $update = $db->where('name', 'google_refreshToken')->update(T_CONFIG, array('value' => ''));
    header("Content-type: application/json");
    echo json_encode($data);
    exit();
}
...
if ($option == 'delete_category') {
    ...
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        ...
        if (!empty($cat)) {
            $data['status'] = 200;
            if ($_GET['type'] == 'song') {
              $db->where('lang_key',$cat->cateogry_name)->delete(T_LANGS);
            } else {
              $db->where('lang_key',$cat->lang_key)->delete(T_LANGS);
            }
            $db->where('id',$cat->id)->delete($types[$_GET['type']]);
...
if ($option == 'remove__curreny') {
    if (!empty($_POST['currency'])) {
        if (in_array($_POST['currency'], $music->config->currency_array)) {
            foreach ($music->config->currency_array as $key => $currency) {
                if ($currency == $_POST['currency']) {
                    if (in_array($currency,array_keys($music->config->currency_symbol_array))) {
                        unset($music->config->currency_symbol_array[$currency]);
                    }
                    unset($music->config->currency_array[$key]);
                }
            }
            if ($music->config->currency == $_POST['currency']) {
                if (!empty($music->config->currency_array)) {
                    $db->where('name', 'currency')->update(T_CONFIG, array('value' => reset($music->config->currency_array)));
                }
            }
            $db->where('name', 'currency_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_array)));
            $db->where('name', 'currency_symbol_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_symbol_array)));
        }
    }
    $data = array(
                'status' => 200
...
if ($option == 'select_currency') {
    if (!empty($_POST['currency']) && in_array($_POST['currency'], $music->config->currency_array)) {
        $currency = secure($_POST['currency']);
        $db->where('name', 'currency')->update(T_CONFIG, array('value' => $currency));
        if (in_array($_POST['currency'], $music->paypal_currency)) {
            $db->where('name', 'paypal_currency')->update(T_CONFIG, array('value' => $currency));
        }
        if (in_array($_POST['currency'], $music->checkout_currency)) {
            $db->where('name', 'checkout_currency')->update(T_CONFIG, array('value' => $currency));
        }
        if (in_array($_POST['currency'], $music->stripe_currency)) {
...
if ($option == 'add-new-currency') {
    $data = array(
                'status' => 400
            );
    if (!empty($_POST['currency']) && !empty($_POST['currency_symbol'])) {
        $music->config->currency_array[] = secure($_POST['currency']);
        $music->config->currency_symbol_array[secure($_POST['currency'])] = secure($_POST['currency_symbol']);
        $db->where('name', 'currency_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_array)));
        $db->where('name', 'currency_symbol_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_symbol_array)));
        $data = array(
                'status' => 200
...
if ($option == 'edit_curreny') {
    if (!empty($_POST['currency']) && !empty($_POST['currency_symbol']) && in_array($_POST['currency_id'], array_keys($music->config->currency_array))) {
        $music->config->currency_array[$_POST['currency_id']] = secure($_POST['currency']);
        $music->config->currency_symbol_array[secure($_POST['currency'])] = secure($_POST['currency_symbol']);
        $db->where('name', 'currency_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_array)));
        $db->where('name', 'currency_symbol_array')->update(T_CONFIG, array('value' => serialize($music->config->currency_symbol_array)));
        $data = array(
...
if ($option == 'get_category_langs') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $types = array('product' => T_PRODUCTS_CATEGORY);
    if (!empty($_GET['type']) && in_array($_GET['type'], array_keys($types)) && !empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        $cat = $db->where('id',Secure($_POST['id']))->getOne($types[$_GET['type']]);
        if (!empty($cat)) {
            $data['status'] = 400;
            $html = '';
            $langs = GetLangDetails($cat->lang_key);
                foreach ($langs as $key => $lang) {
                    foreach ($lang as $key_ => $lang_value) {
                        if ($key_ != 'id' && $key_ != 'lang_key') {
                            $html .= LoadAdminPage('edit-lang/form-list',array(
                                'KEY' => ($key_),
                                'LANG_KEY' => ucfirst($key_),
                                'LANG_VALUE' => $lang_value,
                            ));
                        }
                    }
                }
                $data['lang_key'] = $cat->lang_key;
                $data['status'] = 200;
                $data['html'] = $html;
            header("Content-type: application/json");
            echo json_encode($data);
...
if ($option == 'add_new_category_product') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $types = array('product' => T_PRODUCTS_CATEGORY);
    if (!empty($_GET['type']) && in_array($_GET['type'], array_keys($types))) {
        ...
        foreach (db_langs() as $key => $lang) {
            if (!empty($_POST[$lang])) {
                $insert_data[$lang] = Secure($_POST[$lang]);
                $add = true;
            }
        }
...
if ($option == 'remove_multi_invitation') {
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
    ...
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                ...
                $s3 = ($music->config->s3_upload == 'on' || $music->config->ftp_upload = 'on' || $music->config->spaces = 'on') ? true : false;
                if (!empty($ad)) {
...
if ($option == 'add_announcement') {
    $text           = (!empty($_POST['announcement_text'])) ? base64_decode($_POST['announcement_text']) : "";
    $data['status'] = 400;
    $re_data        = array(
        'text'      => $text,
        'active'    => '1',
        ...
        foreach ($submit_data as $key => $value) {
            ...
            if ($key == 'ftp_upload') {
                if ($value == "on") {
                    ...
                    $update = $db->where('name', 'google_drive')->update(T_CONFIG, array('value' => "off"));
                    $update = $db->where('name', 'spaces')->update(T_CONFIG, array('value' => "off"));
                }
            }
            if ($key == 's3_upload') {
                if ($value == "on") {
...
if ($option == 'save-custom-design-settings') {
    $data     = array('status' => 200);
    $code     = array();
    $code[]   = (!empty($_POST['cheader']))  ? $_POST['cheader']  : "";
    $code[]   = (!empty($_POST['cfooter']))  ? $_POST['cfooter']  : "";
    $code[]   = (!empty($_POST['css'])) ? $_POST['css'] : "";
...
if ($option == 'view_ticket') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        ...
        if (empty($data['message'])) {
            $html = '';
            $ticket = $db->where('event_id',$event->id)->orderBy('id', 'DESC')->get(T_PURCHAES,10);
            if (!empty($ticket)) {
                foreach ($ticket as $key => $value) {
                    $value->user_data = UserData($value->user_id);
                    $music->ticket = $value;
                    $html .= LoadAdminPage('manage-events/ticket_buyers');
                }
            }
...
if ($option == 'load_ticket') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0 && !empty($_POST['load']) && is_numeric($_POST['load']) && $_POST['load'] > 0) {
        $id = secure($_POST['id']);
        ...
        if (empty($data['message'])) {
            $html = '';
            $ticket = $db->where('event_id',$event->id)->where('id',Secure($_POST['load']),'<')->orderBy('id', 'DESC')->get(T_PURCHAES);
            if (!empty($ticket)) {
                foreach ($ticket as $key => $value) {
                    $value->user_data = UserData($value->user_id);
                    $music->ticket = $value;
...
if ($option == 'order_multi_status') {
    if (!empty($_POST['ids']) && !empty($_POST['action_type']) && in_array($_POST['action_type'], array('placed','accepted','packed','shipped','delivered','canceled','delete'))) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                $id = Secure($value);
                $status = secure($_POST['action_type']);
                if ($_POST['action_type'] == 'delete') {
                    $db->where('id',$id)->delete(T_ORDERS);
                }
                else{
                    $db->where('id',$id)->update(T_ORDERS,array('status' => $status));
                    $order = $db->where('id',$id)->getOne(T_ORDERS);
                    $create_notification = createNotification([
                        'notifier_id' => $music->user->id,
                        'recipient_id' => $order->user_id,
                        'type' => 'admin_status_changed',
                        'url' => Secure('customer_order/'.$order->hash_id)
                    ]);
                    $create_notification = createNotification([
                        'notifier_id' => $music->user->id,
                        'recipient_id' => $order->product_owner_id,
                        'type' => 'admin_status_changed',
                        'url' => Secure('order/'.$order->hash_id)
                    ]);
                    $data['status'] = 200;
                }
            }
...
if ($option == 'change_status') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0 && !empty($_POST['status']) && in_array($_POST['status'], array('placed','accepted','packed','shipped','delivered','canceled'))) {
        $id = secure($_POST['id']);
...
if ($option == 'activate-product') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        ...
        if (!empty($product)) {
            $db->where('id',$product->id)->update(T_PRODUCTS,array('active' => 1));
            $create_notification = createNotification([
                'notifier_id' => $music->user->id,
...
if ($option == 'delete-refund') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        $id = secure($_POST['id']);
        $refund = $db->where('id',$id)->getOne(T_REFUND);
        if (!empty($refund)) {
            $db->where('id',$id)->delete(T_REFUND);
            $create_notification = createNotification([
                'notifier_id' => $music->user->id,
                'recipient_id' => $refund->user_id,
                'type' => 'refund_declined',
                'url' => Secure('customer_order/'.$refund->order_hash_id)
            ]);
        }
...
if ($option == 'delete_multi_refund') {
    if (!empty($_POST['ids']) && !empty($_POST['action_type']) && in_array($_POST['action_type'], array('approve','delete'))) {
        foreach ($_POST['ids'] as $key => $id) {
            if (!empty($id) && is_numeric($id)) {
                $refund = $db->where('id',secure($id))->getOne(T_REFUND);
                if (!empty($refund)) {
                    if ($_POST['action_type'] == 'delete') {
                        $db->where('id',$id)->delete(T_REFUND);
                        $create_notification = createNotification([
                        ...
                        $total_final_price = 0;
                        $price = 0;
                        $music->orders = $db->where('hash_id',$refund->order_hash_id)->get(T_ORDERS);
                        foreach ($music->orders as $key => $music->order) {
                            $total_final_price += $music->order->final_price;
                            $price += $music->order->price;
                        }
                        $order = $db->where('hash_id',$refund->order_hash_id)->getOne(T_ORDERS);
                        $user = $db->where('id',$order->product_owner_id)->update(T_USERS,array('balance' => $db->dec($total_final_price)));
                        $user = $db->where('id',$refund->user_id)->update(T_USERS,array('wallet' => $db->inc($price)));
                        $db->where('hash_id',$refund->order_hash_id)->update(T_ORDERS,array('status' => 'canceled'));
                        $create_notification = createNotification([
                            'notifier_id' => $music->user->id,
                            'recipient_id' => $refund->user_id,
...
if ($option == 'delete-review') {
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        $id = secure($_POST['id']);
        $db->where('id',$id)->delete(T_REVIEW);
        $images = $db->where('review_id',$id)->get(T_MEDIA);
        if (!empty($images)) {
            foreach ($images as $key => $value) {
                @unlink($value->image);
                PT_DeleteFromToS3($value->image);
            }
        }
        $data['status'] = 200;
...
if ($option == 'remove_multi_product') {
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                $id = Secure($value);
                $product = GetEventById($id);
                if (!empty($product)) {
...
if ($option == 'remove_multi_event') {
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                $id = Secure($value);
                $event = GetEventById($id);
                if (!empty($event) && isAdmin()) {
                    if (!empty($event->org_image)) {
                        @unlink($event->org_image);
                        PT_DeleteFromToS3($event->org_image);
                    }
...
if ($option == 'update_pages_seo') {
    $arr_seo = json_decode($music->config->seo,true);
    $arr_seo[$_POST['page_name']] = array(
        'title' => secure($_POST['default_title']),
        'meta_description' => secure($_POST['meta_description']),
        'meta_keywords' => secure($_POST['meta_keywords']),
    );
    $db->where('name', 'seo')->update(T_CONFIG, array('value' => json_encode($arr_seo)));
    $data['r_status'] = 200;
    $data['r_page'] = $_POST['page_name'];
    $data['config_seo'] = $music->config->seo;
    header("Content-type: application/json");
    echo json_encode($data);
    exit();
}
if ($option == 'delete-story') {
    $data['status'] = 400;
    if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
...
if ($option == 'delete_multi_story') {
    if (!empty($_POST['ids'])) {
        foreach ($_POST['ids'] as $key => $value) {
            if (!empty($value) && is_numeric($value)) {
                $story = GetStory(Secure($value));
                if (!empty($story)) {
                    @unlink($story->org_image);
                    @unlink($story->org_audio);
                    PT_DeleteFromToS3($story->org_image);
