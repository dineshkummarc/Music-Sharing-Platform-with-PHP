...
if ($option == 'add' && IS_LOGGED) {
	if (!empty($_POST['name']) && !empty($_POST['phone']) && !empty($_POST['country']) && !empty($_POST['city']) && !empty($_POST['zip']) && !empty($_POST['address'])) {
		$id = $db->insert(T_ADDRESS,array('name' => Secure($_POST['name']),
		...
		if (!empty($id)) {
			$data['status'] = 200;
			$data['url'] = getLink("settings/".$music->user->username."/addresses");
...
if ($option == 'edit' && IS_LOGGED) {
	if (!empty($_POST['name']) && !empty($_POST['phone']) && !empty($_POST['country']) && !empty($_POST['city']) && !empty($_POST['zip']) && !empty($_POST['address']) && !empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
		...
		if (!empty($address) && ($address->user_id == $music->user->id || IsAdmin())) {
			$db->where('id',$address->id)->update(T_ADDRESS,array('name' => Secure($_POST['name']),
								                                'phone' => Secure($_POST['phone']),
								                                'city' => Secure($_POST['city']),
								                                'zip' => Secure($_POST['zip']),
								                                'address' => Secure($_POST['address']),
			...
			$data['status'] = 200;
			$data['url'] = getLink("settings/".$music->user->username."/addresses");
			$data['message'] = lang('Your address has been edited successfully');
		}
		else{
			$data['message'] = lang('Please check the details');
		}
