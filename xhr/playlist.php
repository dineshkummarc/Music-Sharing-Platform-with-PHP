	...
	if (!empty($_REQUEST['playlists']) && !empty($_REQUEST['id'])) {
		...
		if (!empty($songData)) {
			...
			if (!empty($explodePlaylistIDS)) {
                ...
                RecordUserActivities('add_to_playlist',array('song_data' => $songData, 'audio_id' => $songData->audio_id));
                $data['status'] = 200;
			} else {
				$data['status'] = 300;
			}
		...
		if (is_numeric($_REQUEST['id'])) {
			...
			if (!empty($playlist)) {
			 	if (empty($_POST['name']) || !isset($_POST['privacy'])) {
			        ...
			        if ($music->config->s3_upload != 'on' && $music->config->ftp_upload != 'on' && $music->config->spaces != 'on' && !file_exists($thumbnail)) {
			        	$errors[] = lang("Error found while updating the playlist avatar, Please try again later.");
			        }
