...
if (!empty($path['page'])) {
	...
	if ($page == 'endpoints' && !empty($path['options'])) {
		...
		$option = (!empty($path['options'][2])) ? $path['options'][2] : '';
        if ($path['options'][1] != 'download_user_info' && $path['options'][1] != 'get-song-info' && $path['options'][1] != 'cashfree' && $path['options'][1] != 'paystack' && $path['options'][1] != 'paysera' && $path['options'][1] != 'iyzipay') {
...
if (file_exists($file_location)) {
    ...
    if (in_array($page, array_keys($seo))) {
        $music->site_title       = $seo[$page]['title'];
        $music->site_description = $seo[$page]['meta_description'];
        $music->keyword     = $seo[$page]['meta_keywords'];
    }
} else if (UsernameExits($page)) {
   require_once "./sources/user.php";
} else if (empty($page)) {
	require_once "./sources/home.php";
    $music->site_title       = $seo['home']['title'];
    $music->site_description = $seo['home']['meta_description'];
    $music->keyword     = $seo['home']['meta_keywords'];
} else if (empty($page)) {
...
if (empty($music->site_content)) {
	require_once "./sources/not-found.php";
    $music->site_title       = $seo['not-found']['title'];
    $music->site_description = $seo['not-found']['meta_description'];
    $music->keyword     = $seo['not-found']['meta_keywords'];
}
