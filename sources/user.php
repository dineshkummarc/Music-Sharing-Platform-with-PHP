...
if (empty($path['options'][1]) || $path['options'][1] == 'activities') {
	...
	if (!empty($getActivties)) {
		...
		foreach ($getActivties as $key => $activity) {
			...
			if ($activity->type == 'created_product') {
				$html .= loadPage("user/product_activity", $getActivity);
			}
			elseif ($activity->type == 'created_event') {
				$html .= loadPage("user/event_activity", $getActivity);
			}
			else{
	...
	$getUserSongs = $db->rawQuery("SELECT * FROM `".T_ACTIVITIES."` WHERE `track_id` IN (SELECT `id` FROM `".T_SONGS."` WHERE `user_id` = '".$userData->id."' AND `price` <> '0') OR `product_id` IN (SELECT `id` FROM `".T_PRODUCTS."` WHERE `user_id` = '".$userData->id."') AND (`type` = 'uploaded_track' || `type` = 'created_product') ORDER BY `id` DESC LIMIT 10;");
	if (!empty($getUserSongs)) {
		$html = "";
		foreach ($getUserSongs as $key => $userSong) {
			$a_id = $userSong->id;
            $record_count++;
            $file_ = 'posts';
            if (!empty($userSong->track_id)) {
            	$userSong = songData($userSong->track_id);
            	$music->dark_wave = $userSong->dark_wave;
				$music->light_wave = $userSong->light_wave;
			...
			$music->songData = $userSong;
			$music->store = true;
			$html .= loadPage("user/".$file_, $userSong->songArray);
		}
	}
}elseif (empty($path['options'][1]) || $path['options'][1] == 'events') {
	$getActivties = getActivties(10, 0, $userData->id,array('events' => 1));
	if (!empty($getActivties)) {
		$html = '';
		foreach ($getActivties as $key => $activity) {
            $record_count++;
