...
function userData($user_id = 0, $options = array()) {
    ...
    $fetched_data->wallet_format  = number_format($fetched_data->wallet, 2);
...
function importImageFromLogin($media) {
    ...
    if (file_exists($file_dir)) {
        if ($music->config->s3_upload == 'on' || $music->config->ftp_upload = 'on' || $music->config->spaces = 'on') {
...
function shareFile($data = array(), $type = 0) {
    ...
    $allowed           = 'jpg,png,jpeg,gif,mp3,mp4,mov,webm,mpeg,3gp,mkv,mk3d,mks';
...
function getActivity($activity_id = 0, $fetch = true) {
    ...
    elseif (!empty($getActivity->product_id)) {
        $userSong = GetProduct($getActivity->product_id);
...
function songData($track_id = 0, $createSession = true, $fetch = true) {
    ...
    if ($music->config->s3_upload == 'on' || $music->config->ftp_upload == "on" || $music->config->spaces == "on" || $music->config->google_drive == "on") {
        ...
        if (!empty($matches)) {
            foreach ($matches as $key => $value) {
                if (!empty($value[2]) && strpos($value[2], 'mp3/320')) {
                    ...
                    try {
                        $youtube = new Madcoda\Youtube(array('key' => $music->config->youtube_key));
...
//        }
        ...
        if (($music->config->s3_upload == 'on' || $music->config->ftp_upload == 'on' || $music->config->spaces == 'on' || $music->config->google_drive == 'on') && !empty($Photo) && is_file($Photo)) {
...
function CheckPaystackPayment($ref)
{
    ...
    $ref = Secure($ref);
    $user = $db->where('id',$music->user->id)->where('paystack_ref',$ref)->getValue(T_USERS,"COUNT(*)");
    if ($user < 1) {
        return false;
    }
    $result = array();
        ...
        if($result){
          if($result['data']){
            if($result['data']['status'] == 'success'){
                $db->where('id',$music->user->id)->where('paystack_ref',$ref)->update(T_USERS,array('paystack_ref' => ''));
                return true;
            }else{
...
function PayUPayment($arParams)
{
    ...
    $arParams["ORDER_HASH"] = hash_hmac("md5", $hashString, $music->config->payu_secret_key);
    ...
    if (empty($curlerr) && empty($curlerrcode)) {
        ...
        if ($parsedXML !== FALSE) {
            ...
            if ($parsedXML->STATUS == "SUCCESS") {
                ...
                $data['error'] = "Error: " . $parsedXML->RETURN_MESSAGE . " [" . $parsedXML->RETURN_CODE . "]";
                $data['error'] .= "Error Details: " . $parsedXML->ERRORMESSAGE . "<br> Error Code: [" . $parsedXML->PROCRETURNCODE . "]";
                if (!empty($payuTranReference)) {
                    $data['error'] .= " [PayU reference number: " . $payuTranReference . "]";
                }
            }
        }
        else{
